...
# He dejado el enlace que tenías en la foto. Si sigue fallando, asegúrate de que en Google 
# el script esté publicado para "Cualquier persona".
...
if opcion == "📥 Registrar/Editar":
    ...
    with st.form("tvc_form", clear_on_submit=True):
        clave = st.text_input("Clave del Producto").strip()
...
# --- SECCIÓN: UBICACIONES ---
    ...
    busqueda = st.text_input("🔍 Buscar por clave:").lower()
...
# --- SECCIÓN: STOCK ACTUAL ---
    ...
    st.header("📋 Inventario de Stock")
    st.dataframe(df, use_container_width=True)

# --- SECCIÓN: DESCARGAR EXCEL ---
elif opcion == "💾 Descargar Excel":
    st.header("💾 Exportar Inventario")
    st.write("Haz clic en el botón para descargar el respaldo en Excel.")
